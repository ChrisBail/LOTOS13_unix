...
! Read all the records:
   ...
   ! write(*,*)
	...
	if(nkrat.lt.krat_min) goto 992
    	...
    	res_loc1=res_it1(niter_loc)
	res_loc2=res_it2(niter_loc)
	w_P_S_diff=wps_it(niter_loc)
	dx_loc=dx_it(niter_loc)
	dy_loc=dy_it(niter_loc)
	dz_loc=dz_it(niter_loc)
    ...
    !write(*,*)' aold=',aold
        ...
        ddold=0
    	do i=1,nkrat
		ddold=ddold+abs(tobkr(i)-trfkr(i))
		!write(*,*)' old:',istkr(i),ipskr(i),tobkr(i),trfkr(i)
	end do
    !write(*,*)' ddold=',ddold/nkrat
		...
		do iix=1,5
			...
			do iiy=1,5
				...
				do iiz=1,5
					...
					izmin=iz
					amax=ank
					!write(*,*)' max:',xmin+dx,ymin+dy,zzz,ank
	281				continue
				end do
			end do
		end do
...
773 format(' ips ','|',' sta ','|',' qua ','|','   dt   ','|','  dist  ','|','  angle')
	...
	do i=1,nkrat
		...
		ddnew=ddnew+abs(tobkr(i)-trfkr(i))
		!write(*,*)' new:',istkr(i),ipskr(i),tobkr(i),trfkr(i)
...
write(*,*)ittt,ntot,' d-old-new=',dold,dnew,' perc=',perc
close(11)

...
integer nmark(100),ngood(500),limit_cycles !CB
...
if(key_ft1_xy2.eq.2) then
    ...
    ! Convert and read  coordinates of the stations
...
open(11,file='../../../DATA/'//ar//'/'//md//'/data/rays0.dat',form='unformatted',access='stream')
...
228	continue
	...
	read(1,*,end=229)xzt,yzt,zzt,nkrat
        ...
        read(1,*,end=229)fini,tini,zzt,nkrat ! Read geogr coordinates CB
        if(nkrat.eq.0) goto 228
        !write(*,*)fini,tini,zold,nkrat
        call SFDEC(fini,tini,0.,xzt,yzt,Z,fi0,tet0) ! con lon/lat to xzt,yzt
    else
        write(*,*)' key_ft1_xy2=',key_ft1_xy2
        stop
    end if
...
!write(*,*) k_star_po
    ...
    do ikr=1,nkrat
        read(1,*)ips,ist,tobs
        !write(*,*)ikr,ips,ist,tob
        if(tobs.le.0)then  !CB 
                new_nkrat=new_nkrat-1 
                cycle 
        end if !CB
        kk=kk+1
        ipskr(kk)=ips
...
!        end do
        ...
        282 continue
        ! Look for the bins around the location to see if goalmax coulf be found CB
        index=0

        limit_cycles=20 ! Limit the number of cycles if goalmax always increase
        ...
        do iix=1,5
            ...
            do iiy=1,5
                ...
                do iiz=1,5
                    ...
                    if(zmax+dz.lt.zlim_up) cycle  ! Make sure that zlim_up is properly set
...
281		     continue
        ...
        if(index.eq.1.and.icycle.le.limit_cycles)then ! CB limit number cycles otherwise inf loop
            ixmax1=ixmax
            iymax1=iymax
            izmax1=izmax
            goto 282
    ...
    ! Compare old and new locations CB
    ...
    ngood=1
    dismin=9999999
    ! Remove obs that are doesn't fulfill distance requirements
    do i=1,nkrat
        ...
        if(ipskr(i).eq.2)res_limit=res_limit*wgs ! This is for S phase CB
        dt=tobkr(i)-trfkr(i)
...
!        end if
    ...
    goto 228
229 close(1)
write(*,*) ,'End of 1D straight line computing' !CB
close(11)
close(12)
close(41)
...
do ivert=1,nvert
    ...
    open(1,file='../../../DATA/'//ar//'/'//md//'/data/rays0.dat',form='unformatted',access='stream')

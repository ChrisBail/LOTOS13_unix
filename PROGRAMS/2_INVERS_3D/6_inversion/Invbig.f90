...
vsdepth=1
call read_3D_ini_v(ar,md,vsdepth)
...
open(3,file='../../../TMP_files/tmp/matr'//it//gr//'.dat',form='unformatted',access='stream')
...
if(nrps(1).ne.0) then
    open(1,file='../../../DATA/'//ar//'/'//md//'/data/link_hor1'//gr//'.dat',form='unformatted',access='stream')
    read(1)link_hor_p
    write(*,*)' link_hor_p=',link_hor_p
    close(1)
    open(1,file='../../../DATA/'//ar//'/'//md//'/data/link_ver1'//gr//'.dat',form='unformatted',access='stream')
    read(1)link_ver_p
...
if(nrps(2).ne.0) then
    open(1,file='../../../DATA/'//ar//'/'//md//'/data/link_hor2'//gr//'.dat',form='unformatted',access='stream')
    read(1)link_hor_s
    write(*,*)' link_hor_s=',link_hor_s
    close(1)
    open(1,file='../../../DATA/'//ar//'/'//md//'/data/link_ver2'//gr//'.dat',form='unformatted',access='stream')
    read(1)link_ver_s
...
! The number of elements in vel_p and vel_s can change from one iteration to another CB
! So we cannot compare vel_? between it and itold CB

if(nrps(1).ne.0) then 
	avdvp=0
	open(11,file='../../../DATA/'//ar//'/'//md//'/data/vel_p_'//it//gr//'.dat')
	! if(iter.gt.1) open(1,file='../../../DATA/'//ar//'/'//md//'/data/vel_p_'//itold//gr//'.dat') CB
	do i=1,nvel_p

		!if(iter.gt.1) then
		!	read(1,*)dvp_old,vvp
		!else ! CB
			dvp_old=0 ! This probably needs to be changed and be taken from anom_3D
			i1=kobr_p(i,1)
			i2=kobr_p(i,2)
			!write(*,*)' i1=',i1,' i2=',i2
			if(i1.eq.0) cycle
...
if(nrps(2).ne.0) then 
	...
	!if(iter.gt.1) open(1,file='../../../DATA/'//ar//'/'//md//'/data/vel_s_'//itold//gr//'.dat') !CB
	do i=1,nvel_s

		!if(iter.gt.1) then
		!	read(1,*)dvs_old,vvs !CB
		!else !CB
			dvs_old=0
			i1=kobr_s(i,1)
			i2=kobr_s(i,2)

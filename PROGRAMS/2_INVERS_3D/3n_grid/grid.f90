...
write(*,*)'number of P rays: ', nrps(1), 'Number of S rays:',nrps(2)
close(1)
...
do iiips=1,2
    ...
    sumtotal=0.
    nonzer=0
    open(1,file='../../../TMP_files/tmp/plotray'//ppss//gr//'.dat',form='unformatted',access='stream')
    do iy=1,nypl
        read(1)((plotray(ix,iy,iz),ix=1,nxpl),iz=1,nzpl)
        !write(*,'(10f7.2)')((plotray(ix,iy,iz),ix=1,nxpl),iz=1,nzpl)
        do iz=1,nzpl
            do ix=1,nxpl
                if(plotray(ix,iy,iz).lt.1.0)cycle
                nonzer=nonzer+1
    ...
    write(*,*) 'nonzer=',nonzer !CB
    write(*,*)' aver ray lenght in one block=',averplot
    write(*,*) plotmin*averplot !CB

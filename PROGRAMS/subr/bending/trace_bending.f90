...
!write(*,*)' straight line CB, tout=',tout

npart_max = int_best(totdist / ds_part_min)  ! max segment part of the ray when bending CB
val_bend0 = val_bend_min * npart_max
!write(*,*)'tout=',tout,'zst=',zst,'tot_dist=',totdist,'ds_part_min',ds_part_min, ' npart_max=',npart_max,' val_bend0=',val_bend0
...
!write(*,*)' tini=',tini
!write(*,*) 'ips',ips,'bend',bend,'nodes',nodes ! CB
...
if(tout.lt.tmin) then  ! Check if new travel time is smaller than old travel time (CB)
        tmin=tout
        nrmin=nodes
        xrmin=xray
        yrmin=yray
...
12 continue
!write(*,*) 'npart_max',npart_max !CB
do nparts=1,npart_max ! Start with one segment ! CB

243	continue
        indexx=0
        dpart=1./nparts ! dpart=1 first iteration CB 
        nodes=nrmin
        !do i=1,nodes
        ...
        do ip=1,nparts ! Loop over the segments CB
...
!write(*,*)' part=',ip,' tini=',tini,' s=',spart(nrpart)
                ...
                tmin=tini
                t_part(ip)=tmin  ! time for segment
                !indexx(ip)=0
                !cycle

! Vertical bending:
                do icase=1,2 ! Do upper bending and lower bending !CB
...
!if(ip.eq.15.and.nparts.eq.16)write(*,*)kod,' yray:',yray(kod),dyray(ipp),' ymin:',yrmin(kod)
        ...
        !write(*,*) 'sdlf,sdlkqnlknlqsnnlnn' !CB
        ind_tot=0
        ttot=0
        do ip=1,nparts

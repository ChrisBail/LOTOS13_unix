...
! this function is made to create a new xray called xtmp between
! two points alongs the ray given by part1 and part 2
! if part1=0. and part2=1. then xtmp=xray
! part 1 and part 2 are ratios along ray_path to compute distance ! CB
! for example 0.5 and 1 gives distance between 50% and 100% of the ray
!nodes=nini
!xray=xini
!yray=yini
!zray=zini
...
do inode=n_A,n_B
        ...
        ztmp(ntmp)=z1
        stmp(ntmp)=s1
        kod_tmp(ntmp)=inode  ! That's the node index being concerned
end do

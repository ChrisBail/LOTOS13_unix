...
aver=0
nav=0
! We only take P phase to compute the residual
do i=1,nkrat
    !write(*,*) ipskr3(i), istkr(i) CB
...
!write(*,*)' aver=',aver
!stop
! We loop to remove P observations residuals bigger than res_loc2
...
if(nav.gt.2) then
    ...
    do i=1,nkrat
        if(ipskr3(i).eq.3) cycle
        if(ngood(i).eq.0) cycle
        dtt=abs(dtk(i)-aver) ! its clever as we remove the average from the residual CB
        !write(*,*)' i=',i,' dtt=',dtt
        if(dtt.gt.dmax) then
            dmax=dtt
            imax=i

...
read(1,*)iter           ! code of the iteration
close(1)
...
! Copy ref_start.dat to data/refmod.dat

i=system('cp ../../../DATA/'//ar//'/'//md//'/ref_start.dat ../../../DATA/'//ar//'/'//md//'/data/refmod.dat ') !CB
...
i=system('mkdir -p ../../../TMP_files/tmp') !CB
i=system('mkdir -p ../../../TMP_files/loc') !CB
...
call read_3D_ini_v(ar,md,vsdepth) ! CB load matrix
call read_z_lim(ar,md)
call read_ini_model(ar,md)
call read_topo(ar)
call read_vref(ar,md)

!write(*,*) 'vp',velocity(5.0,5.0,1.0,1)
!write(*,*) 'vs',velocity(5.0,5.0,1.0,2)
!stop
...
open(1,file='../../../DATA/'//ar//'/'//md//'/data/rays'//it0//'.dat',form='unformatted',access='stream')

open(11,file='../../../DATA/'//ar//'/'//md//'/data/rays'//itt//'.dat',form='unformatted',access='stream')
open(41,file='../../../DATA/'//ar//'/'//md//'/data/resid'//itt//'.dat')
open(14,file='../../../DATA/'//ar//'/'//md//'/data/srces'//itt//'.dat')
open(12,file='../../../TMP_files/tmp/ray_paths_'//itt//'.dat',form='unformatted',access='stream')
...
num_lim=0 !CB
nzt=0 ! Number of Eartquakes read counter CB
nray=0
dis_tot1=0
dis_tot2=0
ntot=0
...
nray_s=0
nray_p=0
kkk=0 ! Counter for events outside 3D_grid_ini

21	continue
    aaa=0 !CB
    bbb=0 !CB
    !write(*,*) nray, nray_p, ntot ! CB
    ...
    !Apply correction to take different grid rotations CB
    ...
    nkr=0
    do ikrat=1,nkrat   ! Start reading all arrivals for one earthquake (CB)
        read(1)ips,ist,tobs_old,tref   ! tref is the theoretical arrival times given by binding procedure in 3D bloc
        ! or in 1D for the first iteration 
    ...
    ! Skip some events if necessary ! CB
    !write (*,*) 'EQ number:',nzt ! CB

    !if(nzt.lt.2900) goto 21 ! CB
    !write (*,*) 'EQ number:',nzt ! CB 
...
!if(nzt.ne.21) goto 21
    ...
    !!!! Go to next event if xzt,yzt and zzt out of bounds !CB
    !write(*,*) xzt,yzt,zzt ! CB
    !write(*,*) 'in block',is_in_3D_block(xzt,yzt,zzt) !CB
    if (.not. is_in_3D_block(xzt,yzt,zzt)) then ! CB
            kkk=kkk+1
            write(*,*) kkk
    ...
    itstep=itstep+1
    vzt1=velocity(xzt,yzt,zzt,1) ! Get velocities 1 means Vp
    vzt2=velocity(xzt,yzt,zzt,2) ! Get Vs
    !write(*,*) vzt1,vzt2,xzt,yzt,zzt ! CB
    !stop ! CB
    !write(*,*)' source:',xzt,yzt,zzt,nkrat ! Source coordinates CB
    nk=0
    ddd1=0
    ddd2=0
    ! START TRACE bending and calculation in 3D CB
    ...
    do ikrat=1,nkrat
        ...
        !write(*,*)xzt,yzt,zzt
        ! Get the traveltime between EQ and station !CB
        !write(*,*) 'sdf3',nkrat ! CB
        !call straight_line(xzt,yzt,zzt,xst,yst,zst,ips, tout) ! CB
        !write(*,*) xzt,yzt,zzt,xst,yst,zst,dist,ikrat ! CB
        
        call trace_bending(xzt,yzt,zzt,xst,yst,zst,ips,tout)
        !tout=2
        !ruta=nodes ! CB
        !if(num_eq.gt.num_lim+200) write(*,*) 'sdf4',nk ! CB
        !write(*,*) 'CBBBB',tout
        !write(*,'(2i6,5f10.3)')ist,nodes,xst,yst,tobs,tout,tobs-tout
        !stop ! C
        
        !write(*,*)' size dtold=',size(dtold)! CB
        !write(*,*)' size xray=',size(xray)! CB
        !write(*,*) 'tout',tout,dtold(ikrat) !CB
        ddd1=ddd1+abs(dtold(ikrat)) ! CB 
        !write(*,*)' ddd1=',ddd1! CB
        !write(*,*)' tobs2' !CB
        ddd2=ddd2+abs(tobs-tout)
        !if(num_eq.gt.num_lim+200) write(*,*)' tobsqsd=' !CB
        
        ! Store distance along ray in the three diections
        nod_all(ikrat)=nodes
        !if(num_eq.gt.num_lim+200) write(*,*) 'sdf41',nk ! CB
        do i=1,nodes
        ...
        !write(*,*)' tobs4' !CB
        !if(num_eq.gt.num_lim+200) write(*,*) 'sdf42',nk ! CB
        
!if(iter.eq.1) then
!call trace_bending(xzt+2,yzt-3,zzt+4,xst,yst,zst,ips,	tobs)
!tobkr(ikrat)=tobs
!end if
! Get incidence angle close to source
        trfkr(ikrat)=tout
        x1=xray(1)
        y1=yray(1)
        z1=zray(1)
        ...
        sinal=hor/tot
        ! Rays parameters ! CB
        px=cosbe*sinal*s0 ! is equal to sinal*s0 in 2D
        py=sinbe*sinal*s0
        pz=cosal*s0
        nk=nk+1
        !if(num_eq.gt.num_lim+200) write(*,*) 'sdf5',nk ! CB
        aaa(nk,1)=px
        aaa(nk,2)=py
        aaa(nk,3)=pz
        aaa(nk,4)=1
        bbb(nk)=tobs-tout
        !if(num_eq.gt.num_lim+200) write(*,*) 'sdf6' ! CB
    ...
    red=100*(ddd1-ddd2)/ddd1 ! Resiudal reduction after computing theo
    !write(*,*)' d1=',ddd1/nkrat,' d2=',ddd2/nkrat,' red=',red


    if(nkr.eq.0) goto 21

    ! Compute residuals for P and S pairs
    do i1=1,nkrat
        bbb(i1)=dt3(i1)
        ...
        do i2=1,nkrat
            ...
            ipskr(i1)=3 ! Assign 3 when we have a P and S pair for a same station CB
...
!
    ...
    ! Compute best location using  function CB
    ! goal_best is initially 0 then updated, with locations
    ! ank is the name of the computed 
    ! dstot is initially 0 in first iter
    !write(*,*) 'CB',ank, goal_best ! CB
    !stop ! CB
    ...
    mp=nkrmax
    np=4
    ! Invert the residual and the matrix (too long for grid search) CB
    ! Finding for correction in the model dx,dy,dz,dt
   ...
   !write(*,*)' disp1=',disp1,' disp2=',disp2,' red=',red
    ...
    xnew=xzt+dxcur  ! Update location CB
    ynew=yzt+dycur
    znew=zzt+dzcur
...
!    write(*,*)' limit=',limit
    ...
    ! Recompute if depth is out of bounds
   ...
   ! write(*,*)' dscur=',dscur,' stepmin=',stepmin
    ...
    ! Re-loop if stepmin is not reached, very important ! CB
    if(dscur.gt.stepmin) goto 331 !
    ...
    !write(*,*)x_best,y_best,depth,nkrat !CB
...
!        end if
    ...
    do i=1,nkrat
        ...
        !write(*,*)ipskr(i),istkr(i),tob_best(i),trf_best(i)!CB
...
do ivert=1,nvert
    ...
    open(1,file='../../../DATA/'//ar//'/'//md//'/data/rays'//itt//'.dat',form='unformatted',access='stream')

...
do iar=1,n_ar
    ...
    kod_param=1
    key_ft1_xy2=1
    open(1,file='../../../DATA/'//ar//'/'//md//'/MAJOR_PARAM.DAT',status='old',err=562) !CB
    do i=1,10000
    ...
    i=system('find ../../../DATA/'//ar//'/'//md//'/data -type f ! -name rays0.dat -delete')
    write(*,*)' DATASET:',ar,' MODEL:',md
    write(*,*)' key_ft1_xy2=',key_ft1_xy2,' kref=',kref

    GOTO 771 ! By pass location CB
...
! Optimization for the reference model (only in case of geographical coordinates)
    ...
    771 continue ! bypass location CB
    ...
    !******************************************************************
    ! This block is deprecated in the new LOTOS version as we work with nodes by def
    !kod_param=1
    !open(1,file='../../../DATA/'//ar//'/'//md//'/MAJOR_PARAM.DAT')
    !do i=1,10000
    ...
    do iter=1,niter
        i=system('rm -fr ../../../TMP_files/tmp/*') !CB
        ...
        i=system('../../2_INVERS_3D/1_locate_3D/locate.exe')
        ...
        do igr=1,nornt
           ...
           ! if(iter.eq.1) then ! CB don't skip this otherwise size problem in matrix 
        write(*,*)' _________________________________________________________'
